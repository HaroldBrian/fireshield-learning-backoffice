...
import { BookOpen, TrendingUp, Clock, Trophy, Calendar, Play, ArrowRight, Video, FileText } from 'lucide-react';
  ...
  useEffect(() => {
    const fetchDashboardData = async () => {
      try {
        ...
        const coursesData = await courseService.getCourses({ limit: 3 });
        setRecommendedCourses(coursesData.data);
        
        // Simuler les inscriptions
        setEnrollments([
          {
            id: '1',
            student: user!,
            course: coursesData.data[0],
            status: 'active',
            progress: 65,
                ...
                {upcomingSessions.map((session) => (
                  <div key={session.id} className="flex items-start space-x-3">
                    <div className={`p-2 rounded-lg ${
                      ...
                      session.type === 'quiz' ? 'bg-yellow-100' : 'bg-green-100'
                    }`}>
                      {session.type === 'video' && <Video className="w-4 h-4 text-blue-600" />}
                      {session.type === 'quiz' && <Trophy className="w-4 h-4 text-yellow-600" />}
                      {session.type === 'project' && <FileText className="w-4 h-4 text-green-600" />}
                    </div>
            ...
            <div className="bg-white rounded-xl shadow-sm p-6">
              ...
              <div className="space-y-4">
                {recommendedCourses.slice(0, 2).map((course) => (
                  <div key={course.id} className="border rounded-lg p-3 hover:bg-gray-50 transition-colors">
